...
public class App {
	public static void main(String[] args) {
		...
		leerIAs(listaIAs,listaDebilidades,listaPaises);
	...
	private static void leerIAs(ListaIAs listaIAs,ListaDebilidades listaDebilidades,ListaPaises listaPaises) {
		try {
			...
			while(leer.hasNextLine()) {
				...
				if(pos!=-1) {
					listaPaises.getPais(pos).ingresarIA(ia);
				}
	...
	private static void leerProgramadores(ListaProgramadores listaProgramadores,ListaPaises listaPaises,ListaUsuarios listaUsuarios) {
		try {
			...
			while(leer.hasNextLine()) {
				...
				int pos3= listaUsuarios.buscar(idProgramador);
				if(pos2!=-1) {
					listaPaises.getPais(pos2).ingresarProgramador(programador);										
				}
				if(pos3!=-1) {
					listaUsuarios.getUsuario(pos3).ingresarProgramador(programador);
				}
